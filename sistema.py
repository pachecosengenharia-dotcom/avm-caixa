...
# 1. Configuracao de Layout e Design
st.set_page_config(page_title="Engenharia de Avaliacoes | AVM", layout="wide")
...
st.markdown('<p class="main-title">Sistema Avancado de Engenharia de Avaliacoes (AVM)</p>', unsafe_allow_html=True)
st.markdown('<p class="subtitle">Analise Estatistica Multifatorial com Indice de Localizacao Urbana</p>', unsafe_allow_html=True)

# 2. Busca automatica de arquivos de dados (.csv)
arquivos_csv = glob.glob("*.csv") + glob.glob("*.CSV")
lista_regioes_arquivos = sorted(list(set(arquivos_csv)))

st.sidebar.header("Parametros de Avaliacao")

if not lista_regioes_arquivos:
    st.error("Nenhum arquivo de base de dados (.csv) foi encontrado no repositorio.")
    st.stop()

nomes_regioes = [os.path.splitext(f)[0].replace("_", " ") for f in lista_regioes_arquivos]
regiao_selecionada_nome = st.sidebar.selectbox("Selecione o Municipio de Avaliacao", nomes_regioes)
...
# 3. Entradas tecnicas do imovel
st.sidebar.subheader("Caracteristicas do Imovel")
area = st.sidebar.number_input("Area Util (m2)", min_value=10.0, max_value=1000.0, value=75.0, step=1.0)
quartos = st.sidebar.slider("Quantidade de Quartos", 0, 5, 2)
vagas = st.sidebar.slider("Vagas de Garagem", 0, 4, 1)
conservacao = st.sidebar.selectbox("Estado de Conservacao (1=Regular, 2=Bom, 3=Excelente)", [1, 2, 3], index=1)

st.sidebar.subheader("Localizacao Relativa")
setor_urbano = st.sidebar.number_input(
    "Indice do Setor Urbano (Fator de Bairro)", 
    min_value=0.1, 
    max_value=10.0, 
    value=1.0, 
    step=0.1
...
if 'Setor_Urbano' not in df_filtrado.columns:
    st.error("A coluna 'Setor_Urbano' nao foi encontrada na planilha.")
    st.stop()

if len(df_filtrado) < 6:
    st.warning("Dados insuficientes para modelagem.")
    st.stop()

# 4. Inteligencia do Modelo Regressivo
...
col2.metric("Intervalo Admissivel Minimo", f"R$ {limite_inferior:,.2f}")
col3.metric("Intervalo Admissivel Maximo", f"R$ {limite_superior:,.2f}")

st.write(f"**Precisao Estatistica do Modelo (R2):** {r2_score*100:.2f}%")
...
def gerar_pdf():
    ...
    story.append(Paragraph("LAUDO DE AVALIACAO TECNICA", estilo_titulo))
...
pdf_data = gerar_pdf()
st.sidebar.download_button(label="Baixar Relatorio", data=pdf_data, file_name="Laudo.pdf", mime="application/pdf")
